function Get-MissingUpdates {
    ...
    $MissingUpdates=$ScanResults.SelectNodes("//Check[@Name='Windows Security
    Updates']/Detail/UpdateData[@IsInstalled='false']")
    foreach ($Update in $MissingUpdates) {
    $Ht = @{ }
    $Properties = $Update | Get-Member -Type Property
    foreach ($Prop in $Properties) {
    $Value = ($Update | Select-Object -expandproperty $Prop.Name)
